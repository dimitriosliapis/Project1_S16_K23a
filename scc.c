...
SCC* estimateStronglyConnectedComponents(ind *index_out, list_node *buffer_out, uint32_t num_nodes, ht_Node *explored, uint32_t version) {
    ...
    for(i = 0; i < num_nodes; i++) {
        ...
        scc = tarjanRecursive(&scc, index_out, buffer_out, num_nodes, explored, version, i, &index, &scc_stack);
    }
    return scc;
}
    ...
    if(index_out[v].num_of_children != 0) {
        ...
        while(k < N){
            ...
            if(search(explored, w, HT_BIG, version) == NOT_FOUND){
                *scc = tarjanRecursive(scc, index_out, buffer_out, num_nodes, explored, version, w, index, scc_stack);
